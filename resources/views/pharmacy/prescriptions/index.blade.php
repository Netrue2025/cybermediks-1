...
@push('styles')
    <style>
        ...
        .rx-rowx {
            display: grid;
            grid-template-columns: 140px 1.2fr 1fr 160px 140px 190px;
            /* Rx | Patient/Doctor | Items | Amount | Status | Actions */
            align-items: center;
            padding: 10px 12px;
            border: 1px solid var(--border);
            border-radius: 10px;
        ...
        @media (max-width: 1100px) {
            ...
            .rx-head,
            .rx-rowx {
                grid-template-columns: 120px 1.2fr 0.8fr 150px 120px 1fr;
            }
        }
...
@section('content')
    <div class="cardx mb-3">
        <div class="d-flex align-items-center gap-2 mb-2">
            ...
            <h5 class="m-0">Prescriptions</h5>
        </div>
        <div class="subtle mb-2">Search and manage prescriptions for this pharmacy</div>
    ...
    <div id="rxList">
        @include('pharmacy.prescriptions._list', ['prescriptions' => $prescriptions])
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            // Optional: live submit on change
            ...
            $('#rxFilter').on('submit', function(e) {
                e.preventDefault();
                $.get(`{{ route('pharmacy.prescriptions.index') }}`, $(this).serialize(), function(html) {
                    $('#rxList').html(html);
                });
            });
