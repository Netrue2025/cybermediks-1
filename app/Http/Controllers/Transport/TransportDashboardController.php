...
class TransportDashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $counts = PharmacyProfile::whereUserCountry($countryId)
        ...
        $pharmacies = User::select('id', 'first_name', 'last_name', 'email')
            ->pharmacies()->inCountry($countryId)
            ->whereHas('pharmacyProfile', fn($q) => $q->where('status', 'pending'))
            ->with(['pharmacyProfile:id,user_id,license_no,operating_license,status,created_at,updated_at'])
            ->latest('id')->get();

        $pending  = (int) ($counts->pending ?? 0);
        $approved = (int) ($counts->approved ?? 0);
        $total = User::pharmacies()->inCountry($countryId)->count();
    ...
    {
        abort_unless($pharmacy->country_id === auth()->user()->country_id, 403);
    ...
    // Reject (only if the pharmacy is in the same country)
